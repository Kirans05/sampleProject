...
const index = () => {
  ...
  const SUMSUB_APP_TOKEN = 'sbx:kca4PM1WgkREdu6oLuYEwg7H.rP3smjx1SY993HVZNdHsqRq8hSHQHh6z'; // Example: sbx:uY0CgwELmgUAEyl4hNWxLngb.0WSeQeiYny4WEqmAALEAiK2qTC96fBad - Please don't forget to change when switching to production
const SUMSUB_SECRET_KEY = 'BQMXPYDDJ9Asr8IXntOc1LIxrxz6EfSi'; // Example: Hej2ch71kG2kTd1iIUDZFNsO5C1lh5Gq - Please don't forget to change when switching to production
...
const genereateSignature = () => {
  console.log('Creating a signature for the request...');

  var ts = Math.floor(Date.now() / 10000);
  const signature = crypto.createHmac('sha256',  "BQMXPYDDJ9Asr8IXntOc1LIxrxz6EfSi");
  signature.update(ts + "post".toUpperCase() + "/resources/applicants/123?fields=info");
  ...
  return (
    <div>
      ...
      <button onClick={genereateSignature}>genereateSignature</button>
    </div>
  )
}
